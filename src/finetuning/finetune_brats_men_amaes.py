#!/usr/bin/env python
"""
Finetune AMAES-pretrained UNet on BraTS-MEN segmentation.
...
from utils.utils import setup_seed, find_checkpoint, load_pretrained_weights
...
def main():
    ...
    p.add_argument("--model_name", default="unet_b")
    ...
    p.add_argument("--loss_fn", default="dicece",
    ...
    p.add_argument("--augmentation_preset", default="basic")
    p.add_argument("--num_devices", type=int, default=1)
    p.add_argument("--num_workers", type=int, default=8)
    p.add_argument("--compile", action="store_true")
    p.add_argument("--compile_mode", default=None)
    p.add_argument("--experiment", default="amaes_finetune_bratsmen")
    ...
    experiment_name = f"amaes_{run_type}_{args.model_name}"

    save_dir = os.path.join(args.save_dir, "models", "finetuning",
                            "amaes", args.model_name)
    ...
    wandb_logger = L.pytorch.loggers.WandbLogger(
        ...
        tags=["finetuning", "amaes", "brats-men", run_type],
    )
