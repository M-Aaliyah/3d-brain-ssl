#!/usr/bin/env python
"""
Full joint HP search for AMAES finetuning on BraTS-MEN.
...
Selection: maximise val/dice (mean foreground Dice, background is ignored)
...
Writes:
    artifacts/hp_search/amaes/results.json
    artifacts/hp_search/amaes/best_config.json
"""
...
from utils.utils import setup_seed, load_pretrained_weights
...
class ValDiceTracker(L.Callback):
    """Records val/dice each epoch for HP selection."""
    def __init__(self):
        super().__init__()
        self.epoch_dices = []
...
def run_config(run_tag, lr, wd, loss_fn, lr_scheduler,
    ...
    config = {
        "experiment": f"amaes_hp_{run_tag}",
        "run_type": "finetune",
        "task_type": task_type,
    ...
    wandb_logger = L.pytorch.loggers.WandbLogger(
        ...
        name=f"amaes_hp_{run_tag}",
        save_dir=os.path.join(REPO_ROOT, "wandb"),
        log_model=False,
        tags=["hp_search", "amaes", "brats-men"],
        config={
...
def main():
    ...
    parser.add_argument("--augmentation_preset", type=str, default="basic")
    ...
    parser.add_argument("--resume_from", type=str, default=None,
                        help="Path to results.json to skip completed runs")
    ...
    search_dir = os.path.join(args.save_dir, "hp_search", "amaes")
    ...
    # Sequential mode run all configs in a single process
    ...
    print(f"\n{'='*65}")
    print(f"AMAES HP search — {n} configs (sequential mode)")
    print(f"  LR: {args.lr_values}")
    print(f"  WD: {args.wd_values}")
    print(f"  loss_fn: {args.loss_fns}")
    print(f"  scheduler: {args.schedulers}")
    ...
    best_config = {
        ...
        "next_step": (
            f"Run finetune_brats_men_amaes.py with "
    ...
    print(f"\n{'='*65}")
    print("AMAES HP SEARCH COMPLETE")
    print(f"\nTop configs:")
