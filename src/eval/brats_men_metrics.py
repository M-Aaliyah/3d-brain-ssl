...
# NumPy 2.x compatibility for older libraries
...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    for l in range(len(label_values)):
        ...
        metric_df[numeric_cols] = metric_df[numeric_cols].replace(
            [np.inf, -np.inf],
            374
        )
    ...
    results_df[numeric_cols] = results_df[numeric_cols].replace([np.inf, -np.inf], 374)
